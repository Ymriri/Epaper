...
class GenerateDay:
    def __init__(self, city: str = "101280101", img_path: str = "", user: str = "ymri"):
        """
        date = {
            # 相对路径
            "img": j["img"],
        ...
        try:
            ...
            with open("lunar.json", "r") as f:
                self.lunar = json.load(f)
            # 去年节假日和时间
            with open("allDay.json", "r") as f:
...
def parse_args():
    ...
    parser.add_argument('--user', type=str, help='用户', default="ymri", required=False)
...
if __name__ == "__main__":
    ...
    user = args.user
    city = args.city
    g = GenerateDay(city=city, img_path=img_path, user=user)
